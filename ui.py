...
from state import AppState, AppStates, GLOBAL_STATE
...
class App(ctk.CTkFrame):
    ...
    def __init__(self, title: str, size: tuple) -> None:
        GLOBAL_STATE.current_state = AppStates.STOPPED.value
    ...
    def __render_home_screen(self):
        ...
        self.header_label = ctk.CTkLabel(self.__master,
                                         text=GLOBAL_STATE.current_state,
    ...
    def __refresh_home_screen(self):
        ...
        if GLOBAL_STATE.current_state == AppStates.PAUSED.value:
            return

        self.header_label.configure(text=GLOBAL_STATE.current_state + ' ' + str(session_number) + ' de ' + str(sessions_length))
    ...
    def toggle(self):
        match GLOBAL_STATE.current_state:
            case AppStates.STOPPED.value:
                # NOTE: Começa a sessão
                GLOBAL_STATE.current_state = AppStates.STOPPED.value
                GLOBAL_STATE.current_state = AppStates.IN_SESSION.value
                self.__refresh_all_screen()
                self.__draw_restart_button()
            case AppStates.PAUSED.value:
                # NOTE: Retoma a sessão/descanso
                GLOBAL_STATE.current_state = GLOBAL_STATE.last_state
                self.__refresh_all_screen()
                self.__draw_restart_button()
            case AppStates.IN_SESSION.value:
                # NOTE: Pausa a sessão/decanso
                GLOBAL_STATE.last_state = AppStates.IN_SESSION.value
                GLOBAL_STATE.current_state = AppStates.PAUSED.value
            case AppStates.IN_REST.value:
                GLOBAL_STATE.last_state = AppStates.IN_REST.value
                GLOBAL_STATE.current_state = AppStates.PAUSED.value
        ...
        match GLOBAL_STATE.current_state:
            case AppStates.IN_SESSION.value | AppStates.IN_REST.value:
                try:
                    ...
                    try:
                        GLOBAL_STATE.last_state = AppStates.IN_SESSION.value
                        GLOBAL_STATE.current_state = AppStates.IN_REST.value
